...
//
// ── 실측 결과 (2026-08-31, MLB 2023~2025, 평가 6,965경기) ─────────────────────
//
// 고리1  총득점 -> 1점차 확률: 강하게 성립
//   5점 이하 46.8% / 6~7점 29.4% / 8~9점 25.2% / 10~12점 14.7% / 13점+ 21.4%
//   32%p 스윙이다. 축구 무승부가 Elo 격차에 따라 8.0%p 움직인 것의 4배.
//   -> 앞선 measure_baseball.ts의 "1점차는 예측 대상이 아니라 상수(~28%)"는 틀린 결론이었다.
//      상수가 아니라, 전력차라는 엉뚱한 축으로 잘랐기 때문에 평평해 보였던 것이다.
//
// 고리2  사전정보 -> 총득점: 여기서 끊어진다
//   팀 최근 20경기 득점/실점으로 만든 기대총득점 vs 실제: r = +0.0837, R^2 = 0.70%
//   5분위     기대 7.83 / 8.51 / 8.96 / 9.41 / 10.20  (스프레드 2.37)
//             실제 8.58 / 8.60 / 9.12 / 8.94 /  9.55  (스프레드 0.97)
//             1점차 28.6 / 29.6 / 27.8 / 27.4 / 28.7%  <- 완전히 평평
//
// 고리3  이어붙인 승1패 3택: 1/4 통과(사실상 실패)
//   튜닝 계수가 +0.006~-0.004로 0에 수렴하고 적중률 변화는 0.00%p.
//
// 상한   총득점을 완벽하게 안다고 가정하면 42.1% (아무것도 모를 때 35.8%)
//   총득점       n     승     1     패    최적픽  적중률
//   5점 이하   1696  28.8%  46.8%  24.5%   1     46.8%
//   6~7점      1294  37.6%  29.4%  32.9%   승    37.6%
//   8~9점      1204  36.9%  25.2%  38.0%   패    38.0%
//   10~12점    1338  42.5%  14.7%  42.8%   패    42.8%
//   13점+      1433  35.0%  21.4%  43.6%   패    43.6%
//
//   이게 이 방향의 결론이다. 완벽한 총득점 예측이 벌어주는 최대치가 6.3%p뿐이고,
//   현재 설명력은 R^2 0.70%다. 선발투수(statsapi가 과거 경기 18/18 전부 제공 - 확인함)를
//   넣어 설명력을 크게 올린다 해도 그 6.3%p 중 일부만 가져온다. 야구 게임 단위 총득점
//   예측의 알려진 난이도를 감안하면 실제 도달치는 36~38% 수준일 것으로 보이는데,
//   이건 추정이지 측정이 아니다. 확실한 건 상한 42.1%라는 사실뿐이다.
//   승1패 종목 자체에 배당도 없어(wisetoto bs2/bs5/pt1 배당값 0건, 실측) 보정할 수단도 없다.
//
// 참고   승패 2택은 4분할 54.41~55.67%, 무조건 홈승 기준선 53.14%
//   Elo가 버는 건 +1.3~2.5%p. 이쪽은 프로토 승부식에 배당이 있을 가능성이 남아 있어
//   (야구토토와 다른 상품이라 미확인) 별도로 볼 여지는 있다.
// ──────────────────────────────────────────────────────────────────────────────

...
// ── 실측 결과 (2026-08-31, football-data 4리그 4시즌 / 배당 보유 3,512경기) ──────
//
// 4분할 test 로그손실 평균의 증분 기여:
//   홈어드밴티지 추가   -0.0083   실질적 개선
//   최근폼 추가         -0.0001   사실상 0
//   H2H 추가            -0.0007   사실상 0
//   배당 블렌딩 추가    -0.0154   모델 항 어느 것보다 크다
//   E -> A (배당만)     -0.0272   배당 단독이 우리 전체 모델을 이긴다
//
// 분할별 적중률 (E=현재 모델, A=배당만):
//   0.5  E 52.33%  A 54.61%      0.7  E 51.61%  A 53.32%
//   0.6  E 52.46%  A 54.38%      0.8  E 51.07%  A 53.34%
//   배당이 4개 분할 전부에서 약 2%p 앞선다.
//
// 핵심 발견 세 가지
//
// 1) 모델의 예측력은 사실상 C(Elo + 홈어드밴티지)에서 끝난다.
//    폼과 H2H를 더해도 로그손실이 각각 0.0001, 0.0007밖에 안 움직인다.
//    현행 가중치(폼 60, H2H 50)는 "상관계수 비율 Elo 0.40 : 폼 0.27 : H2H 0.23"에서
//    나왔는데, 그 상관은 Elo와의 겹침을 다시 센 것이었다는 뜻이다.
//
// 2) H2H는 4개 분할 전부에서 적중률을 떨어뜨린다.
//      0.5  52.51 -> 52.33      0.7  51.90 -> 51.61
//      0.6  52.67 -> 52.46      0.8  51.92 -> 51.07
//    로그손실은 -0.0007 좋아진다. 시즌누적 xG와 정확히 같은 패턴 - 확률은 조금
//    부드러워지는데 정작 픽은 나빠진다. 앞서 measure_h2h_weight.ts에서 "50이 로그손실
//    최적"이라고 본 것은 전체 구간(가중치를 고른 그 데이터)에서 잰 값이었고, 홀드아웃을
//    적용하면 그림이 달라진다. 다만 차이가 0.2~0.9%p로 작고 test n이 703~1,756이라
//    단독으로 제거를 정당화할 만큼 크지는 않다. 판단은 사용자 몫으로 남긴다.
//
// 3) 캘리브레이션(ECE)은 오히려 우리 모델이 좋다(E 0.0107~0.0271 vs A 0.0202~0.0302).
//    즉 우리 모델은 과신하는 게 아니라 정보가 적은 것이다. 이건 중요한 구분인데,
//    과신이면 온도조정으로 고칠 수 있지만 정보 부족은 새 정보로만 메워진다.
//
// 주의: 모든 arm이 같은 무승부 모델을 공유하므로 이 표는 강도 항의 기여만 분리한다.
// ──────────────────────────────────────────────────────────────────────────────
...
async function main() {
  ...
  const steps: [string, string, string][] = [
    ...
    ["D", "E", "H2H 추가"],
    ["E", "F", "배당 블렌딩 추가"],
    ["E", "A", "우리 모델 -> 배당만 (음수면 배당이 우리보다 낫다는 뜻)"],
  ];

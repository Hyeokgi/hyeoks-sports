# 회차별 투표율/모델예측/실제결과 비교를 구글시트 "HYEOKS_회차분석" 탭에 기록(매번 clear+재기록).
#
# 2026-08-22: hyeoks-sports-engine 레포에서 이 저장소로 이관했다. 원래 엔진 레포에 있었으나
# (a) 이 스크립트가 이미 이 앱의 API를 소비하고 있었고 (b) 1~41회차 원본 데이터가 엔진 레포에만
# 있어 앱 쪽 백테스트가 그 데이터의 존재조차 모르는 상태였다. 데이터와 소비자를 같은 곳에 둔다.
#
# 1~41회차: betman 과거기록 파일(투표 vs 실제결과, 2축 - 그 시절엔 우리 모델이 없었음).
# 42회차~: 이 앱의 API(투표+모델예측+실제결과, 3축 - round_results/round_vote_share 정산 인프라 이후).
# 두 구간은 데이터 출처가 달라 같은 시트에 컬럼을 맞춰 이어붙인다.
import json
import os
...
APP_BASE_URL = os.environ.get("WORKER_BASE_URL", "https://kleague-toto-predictor.hyeoks.workers.dev")
...
def build_historical_rows():
    ...
    재구성으로 채웠다(574경기 중 63건, 나머지는 EPL/세리에A라 당시엔 다중시즌 Elo 히스토리가
    없어 재구성 불가였다 - 2026-08-22에 두 리그를 편입했으므로 추후 재구성 가능).
    """
    rounds = json.loads((SEED / "history_rounds_1_41.json").read_text(encoding="utf-8"))
    votes = json.loads((SEED / "history_votes_1_41.json").read_text(encoding="utf-8"))
    retro_path = SEED / "history_retro_picks_1_41.json"
    retro_lookup = {}
    if retro_path.exists():
        for row in json.loads(retro_path.read_text(encoding="utf-8")):
    ...
    for r in rounds:
        ...
        for m in r["matches"]:
            ...
            if not v or v.get("voteWin") is None:
                continue
            # 팀명 교차확인 - betman 원본 자체에 seq 정렬이 어긋난 회차가 있어 조용히 섞이면 안 된다
...
def build_app_rows():
    """42회차~: 이 앱의 API 기반, 실제결과가 확정된(정산된) 경기만 3축으로 기록."""
    rows = []
    try:
        res = requests.get(f"{APP_BASE_URL}/api/rounds", timeout=10)
    ...
    for round_info in rounds:
        if not round_info.get("round_no_confirmed"):
            continue
        round_id, round_no = round_info["id"], round_info["round_no"]
        try:
            detail = requests.get(f"{APP_BASE_URL}/api/rounds/{round_id}", timeout=10)
            detail.raise_for_status()
            matches = detail.json().get("matches", [])
        except Exception as e:
